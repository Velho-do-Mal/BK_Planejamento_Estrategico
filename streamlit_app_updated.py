...
def _month_labels_for_okr(o: OKR) -> List[str]:
    labels = []
    for m in o.meses[:36]:
        labels.append(f"{int(m.mes):02d}/{int(m.ano)}")
    return labels
...
with tabs[4]:
    ...
    if not planning.okrs:
        ...
        if okr_obj:
            ...
            prev = [float(m.previsto) for m in okr_obj.meses[:36]]
            real = [float(m.realizado) for m in okr_obj.meses[:36]]
            ...
            with st.expander("Legenda dos meses (M01..M36) para esta OKR"):
                st.write(pd.DataFrame({"Coluna": [f"M{k+1:02d}" for k in range(36)], "Mês": labels}))
